...
| `vbase_default`        |         | `Dict` |       | always | Instruction to set the vbase at a number of buses for non-dimensionalization |
...
This is a special case of `shunt` with its own data category for easier tracking, sharing the same fields as `shunt` (_i.e._ an alias to `shunt`).
